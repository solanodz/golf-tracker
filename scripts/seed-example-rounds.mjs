/**
 * Genera supabase/seed-example-rounds.sql con 5 rondas por cancha.
 * Ejecutar: node scripts/seed-example-rounds.mjs
 */
...
const allDates = schedules.flatMap((s) => s.rounds.map((r) => `'${r.date}'::date`));
...
BEGIN
  ...
  LOOP
    ...
    VALUES (v_user_id, v_course_id, meta.played_on, 'completed', ${HANDICAP})
...
writeFileSync(join(__dirname, "../supabase/seed-example-rounds.sql"), sql);
